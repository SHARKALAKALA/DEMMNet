...
#summary of changes: 
#full code integration with all_B2 updates at param level instead of group, moves slow weights into state dict (from generic weights),
#supports group learning rates (thanks @SHolderbach), fixes sporadic load from saved bisenetv1 issues.
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if len(state) == 0:   #if first time to run...init dictionary with our desired entries
                        ...
                        #print("Initializing slow buffer...should not see this at load from saved bisenetv1!")
                    state['step'] = 0
                    state['exp_avg'] = torch.zeros_like(p_data_fp32)
                    state['exp_avg_sq'] = torch.zeros_like(p_data_fp32)
                    if amsgrad:
                        # Maintains max of all_B2 exp. moving avg. of sq. grad. values
                ...
                if amsgrad:
                    # Maintains the maximum of all_B2 2nd moment running avg. till now
