...
def save_ckpt(logdir, model, kind, ep='', miou='', ca=''):
    if kind == 'end':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_end.pth'))
    elif kind == 'best':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, f'model_best.pth'))
    elif kind == 'best_miou':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, f'model_best_{ep}_miou:{miou}.pth'))
    elif kind == 'best_ca':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, f'model_best_{ep}_ca:{ca}.pth'))


def load_ckpt(logdir, model, kind, strict=False):
    if kind == 'end':
        save_pth = os.path.join(logdir, 'model_end.pth')
        model.load_state_dict(torch.load(save_pth), strict=strict)
    elif kind == 'best':
        # save_pth = os.path.join(logdir, 'model_best.pth')
        save_pth = os.path.join(logdir, 'model_best.pth')
        model.load_state_dict(torch.load(save_pth), strict=strict)
        # loaded_state = torch.load(save_pth, map_location='cuda:0')
        # bisenetv1.load_state_dict(loaded_state)
    return model
