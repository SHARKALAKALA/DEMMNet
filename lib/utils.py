...
def load_model(model, model_file, depth_input=False, is_restore=False):
    ...
    if isinstance(model_file, str):
        ...
        if 'bisenetv1' in state_dict.keys():
            state_dict = state_dict['bisenetv1']
    else:
    ...
    if len(missing_keys) > 0:
        logger.warning(
            'Model keys:{}, state dict: {}, missing key(s) in state_dict: {}'.format(len(own_keys), len(ckpt_keys),
